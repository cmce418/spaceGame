...
function main( )
	...
	local background1 = display.newImage("images/space2.png")
	background1.x = centerX
	background1.y = centerX

	local background2 = display.newImage("images/space2.png")
...
--Sprite Explosion Animation
	...
	leftArrow.x = display.contentWidth - 300
	leftArrow.y = display.contentHeight - 3 
	...
	rightArrow.x = display.contentWidth -250
	rightArrow.y = display.contentHeight -3
	...
	shootBtn.y = display.contentHeight - 3
	...
	--Ship
	ship = display.newImage("images/ship3.png")
	ship.x = centerX
	ship.y = display.contentHeight - 50
	ship.width = 80
	ship.height = 80
	physics.addBody(ship, "static", {density = 3, friction = 0, bounce = 0.8}); 

	
	--score
	scoreText = display.newText("Score : "..score,190, 5, native.systemfont, 30)

	--life
	lifeText = display.newText("Life : "..life,4, 5, native.systemfont, 30)
	--enemy
	function enemySpawn()
		function spawn()
				RandomX = math.random(40, display.contentWidth - 20)
				if ( math.random(1,5) == 1 or math.random(1,5) == 2 ) then
					...
					asteroid.width = 80
					asteroid.height = 80
					asteroid.rotation = math.random(30, 190)
					asteroid.type = "enemy"
					physics.addBody(asteroid, "dynamic", {friction = 0})
					transition.to( asteroid, {time=3000, y= display.contentHeight + 400})
				else
		...
		if(event.phase == "began") then
			if(event.other.type == "enemy") then
				...
				if(life == 0) then
					...
					gameover = display.newText("Game Over", centerX,  centerY - 400, 0, 180, native.systemfont, 50)
					transition.to( gameover, {time=1200, x = centerX ,y=centerY} )
				end
			end
		end
	end
...
function startMenu()
	menu = display.newImage("images/menu4.png")
	menu.height = display.contentHeight + 100

	startBtn = display.newText("Start", display.contentCenterX -40, display.contentCenterY,"SHERWOOD", 50 )
	startBtn:setFillColor(124, 183, 211)

	creditsBtn = display.newText("Credits", display.contentCenterX -40, display.contentCenterY + 70,"SHERWOOD", 50)
	creditsBtn:setFillColor(124, 183, 211)

	exitBtn = display.newText("Exit", display.contentCenterX -40, display.contentCenterY + 140,"SHERWOOD", 50 )
	...
	function showCredits()
		...
		credits = display.newImage("images/user.png", 0, display.contentCenterY + 300)
...
function splash()
	splashScreen = display.newImage( "images/kmcc.jpg" )
	splashScreen.height  = 600
